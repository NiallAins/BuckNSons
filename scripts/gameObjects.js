...
function BuckDead(xIn, yIn, dxIn, dyIn) {
	...
	gameObjs.gemBar.totalGreen += gameObjs.gemBar.green;
	gameObjs.gemBar.totalBlue += gameObjs.gemBar.blue;
	gameObjs.gemBar.totalRed += gameObjs.gemBar.red;
}
...
function Death(state) {
	...
	Death.prototype.draw = function() {
		if (this.restart) {
			if (this.fade === 0) {
				Assets.sounds.soundtrack.pause();
        		Assets.sounds.soundtrack.currentTime = 0;
			...
			ctx.save();
				...
				if (this.timer > 1400) {
					ctx.save();
						ctx.globalAlpha = (this.timer - 1400) / 100;
						this.credit5.draw(-Level.x + 280, -Level.y + 80);
					ctx.restore();
				} else if (this.timer > 1300) {
					ctx.save();
						ctx.globalAlpha = 1 - ((this.timer - 1300) / 100);
						this.credit4.draw(-Level.x + 320, -Level.y + 130);
					ctx.restore();
				} else if (this.timer > 1200) {
					ctx.save();
						ctx.globalAlpha = (this.timer - 1200) / 100;
						this.credit4.draw(-Level.x + 320, -Level.y + 130);
					ctx.restore();
				} else if (this.timer > 1200) {
					ctx.save();
						ctx.globalAlpha = 1 - ((this.timer - 1200) / 100);
	...
	Death.prototype.step = function() {
		if (this.timer > 400 && this.timer < 1400 && (kb.left || kb.right || kb.up || kb.down)) {
			this.timer = 1400;
		}
		if (this.restart) {
...
function ShawnDead(xIn, yIn, sprIn, xHIn, yHIn, sprHIn) {
	...
	ShawnDead.prototype.step = function() {
		if (this.alive === 1) {
			...
			this.xH -= (this.xH - (-Level.x + 610)) / 10;
			this.yH -= (this.yH - (-Level.y + 190)) / 10;
		}
	}
...
function GemBar() {
	...
	this.spr = {
		...
		green : new Sprite(Assets.sprites.bigGemGreen, 40, 800, false),
		blue : new Sprite(Assets.sprites.bigGemBlue, 40, 800, false),
		gray : new Sprite(Assets.sprites.badGem, 40, 800, false),
		monk : new Sprite(Assets.sprites.monkScore, 126, 0)
	}
	this.spr.blue.frNum += 4;
	this.spr.red.frNum += 6;
	this.spr.gray.frNum += 2;
}
	...
	GemBar.prototype.drawScore = function() {
		ctx.save();
			ctx.fillStyle = "#FFF";
			ctx.font = "20pt Monospace";

			this.spr.monk.draw(-Level.x + 525, -Level.y + 50);
			ctx.fillText(this.totalBoss + " / 3", 670, 130);
			...
			ctx.fillText(this.totalLost, 705, 475);

			ctx.fillText("=", 530, 550);
			ctx.font = "35pt Monospace";
			ctx.fillText(Math.floor(((this.totalBoss + this.totalGreen + this.totalRed + this.totalBlue - this.totalLost) * 100) / 88) + "%", 640, 560);
		ctx.restore();
	}
